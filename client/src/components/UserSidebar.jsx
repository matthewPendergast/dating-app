    ...
    return (
        <div className="flex flex-col max-h-[100vh]">
            ...
            <div className="flex h-[5vh] min-h-[2.5rem]">
                <div 
                    className={`${styles.button} ${activeTab === "likes" ? "brightness-90" : "brightness-100"}`}
                    onClick={() => setActiveTab("likes")}>
                    <p className="font-semibold">Likes</p>
                    <p className="px-2 py-1 rounded-[50%] font-semibold text-white bg-[#fe94bc]">{data.likes.length}</p>
                </div>
                <div 
                    className={`${styles.button} ${activeTab === "matches" ? "brightness-90" : "brightness-100"}`}
                    onClick={() => setActiveTab("matches")}>
                    <p className="font-semibold">Matches</p>
                    <p className="px-2 py-1 rounded-[50%] font-semibold text-white bg-[#fe94bc]">{data.matches.length}</p>
                </div>
            </div>
